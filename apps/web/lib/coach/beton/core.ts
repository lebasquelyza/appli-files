...
type StrengthFocus = "full" | "bas_quads" | "bas_iscios_glutes" | "haut_push" | "haut_pull" | "haut_mix" | "bras_core";
const FOCUS_LABEL: Record<StrengthFocus, string> = {
  full: "Full body", bas_quads: "Bas (quadris)", bas_iscios_glutes: "Bas (ischios/fessiers)",
  haut_push: "Haut (poussée)", haut_pull: "Haut (tirage)", haut_mix: "Haut (mix)", bras_core: "Bras & Core",
};
function makeFocusPlan(n: number, goalKey: string): StrengthFocus[] {
  const g = (goalKey || "general").toLowerCase();
  if (g === "hypertrophy") { if (n<=1) return["full"]; if(n===2) return["bas_iscios_glutes","haut_mix"];
    if(n===3) return["bas_quads","haut_push","haut_pull"]; if(n===4) return["bas_quads","haut_push","bas_iscios_glutes","haut_pull"];
  ...
  if (g === "strength") { if (n<=1) return["full"]; if(n===2) return["bas_quads","haut_push"];
    if(n===3) return["bas_quads","haut_pull","haut_push"]; if(n===4) return["bas_quads","haut_push","bas_iscios_glutes","haut_pull"];
  ...
  if (g === "fatloss") { if (n<=1) return["full"]; if(n===2) return["bas_iscios_glutes","haut_mix"];
    if(n===3) return["full","haut_mix","bas_quads"]; if(n===4) return["bas_quads","haut_mix","bas_iscios_glutes","haut_pull"];
  ...
  if (n<=1) return["full"]; if(n===2) return["bas_iscios_glutes","haut_mix"]; if(n===3) return["bas_quads","haut_push","haut_pull"];
  if (n===4) return["bas_quads","haut_push","bas_iscios_glutes","haut_pull"]; if(n===5) return["bas_quads","haut_push","bas_iscios_glutes","haut_pull","full"];
  return["bas_quads","haut_push","bas_iscios_glutes","haut_pull","full","haut_mix"];
}

/* ========================= API principale ========================= */
export function planProgrammeFromProfile(profile: ProfileInput = {}, opts?: PlanOptions): { sessions: AiSessionT[] } {
  ...
  for (let i = 0; i < maxSessions; i++) {
    ...
    const variant = i % 3, labelABC = ["A","B","C"][variant];
    ...
    const title = profile.prenom
      ? (singleNoDay ? `Séance pour ${profile.prenom}${focusSuffix}` : `Séance pour ${profile.prenom} — ${dayLabel}${focusSuffix}`)
      : (singleNoDay ? `${defaultBaseTitle(type)}${focusSuffix}` : `${defaultBaseTitle(type)} — ${dayLabel}${focusSuffix}`);

    const exos = type === "cardio" ? buildCardio(ctx, variant)
    ...
    sessions.push({
      ...
      title, type: type as WorkoutType, date: dateStr,
      plannedMin: minutes, intensity: type === "hiit" ? "élevée" : "modérée",
      exercises: exos,
    } as AiSessionT);
  }
  return { sessions };
...
function inferMaxSessions(text?: string | null): number | undefined {
  if (!text) return undefined; const s = String(text).toLowerCase();
  const numMatch = s.match(/\b(\d{1,2})\s*(x|fois|jours?)\b/); if (numMatch) { const n = parseInt(numMatch[1],10); if (!Number.isNaN(n)) return clamp(n,1,6); }
  if (/toute?\s+la\s+semaine|tous?\s+les\s+jours/.test(s)) return 6;
  const days = extractDaysList(s); if (days.length) return clamp(days.length,1,6);
  return undefined;
}
function extractDaysList(text?: string | null): string[] {
  if (!text) return []; const s = String(text).toLowerCase(); const out: string[] = []; const push = (d:string)=>{ if(!out.includes(d)) out.push(d); };
...
function normalizeItems(list?: string[]): Items {
  ...
  const has = (pat:RegExp)=> txt.some(s=>pat.test(s));
  return {
    bands: has(/elas|élast|band/), kb: has(/kb|kettlebell/), trx: has(/trx|suspens/),
    bench: has(/banc|bench/), bar: has(/barre|barbell/), db: has(/halter|haltère|dumbbell|halt/),
    bike: has(/velo|vélo|bike|spinning/), rower: has(/rameur|row/), treadmill: has(/tapis|tread/),
  };
}
...
function buildCardio(ctx: Ctx, variantIdx: number): NormalizedExercise[] {
  ...
  const main: NormalizedExercise = (variantIdx % 2 === 0)
...
/* ========================= Musculation ========================= */
type PoolItem = { name: string; need?: ("bar"|"db"|"bench"|"machine"|"kb"|"bands"|"trx")[]; fallback?: string; area?: "bas"|"haut"|"dos"; kind?: "main"|"assist"|"iso"|"core"; contra?: (inj: Injuries) => boolean; };
...
function baseRest(goal:string){ const g=(goal||"general").toLowerCase(); if(g==="strength") return 120; if(g==="hypertrophy") return 75; if(g==="fatloss") return 45; return 60; }
function defaultSets(level: Ctx["level"], isBW=false){ if(isBW) return level==="avance"?4:3; return level==="avance"?4:level==="intermediaire"?3:2; }
function defaultReps(goal:string, main=false){ const g=(goal||"general").toLowerCase(); if(g==="strength") return main?"3–5":"4–6"; if(g==="hypertrophy") return main?"6–10":"10–15"; if(g==="fatloss") return main?"10–12":"12–20"; return main?"6–10":"8–12"; }
function estimateSetSeconds(goal:string){ const g=(goal||"general").toLowerCase(); if(g==="strength") return 25; if(g==="hypertrophy") return 30; if(g==="fatloss") return 25; return 28; }
function estimateExerciseMinutes(ex:NormalizedExercise, goal:string){ const sets=ex.sets??3; const setSec=estimateSetSeconds(goal); const restSec=parseRestToSec(ex.rest)||baseRest(goal); return (sets*(setSec+restSec))/60; }
function parseRestToSec(r?:string){ if(!r) return 0; const m=r.match(/(\d+)\s*-\s*(\d+)s/); if(m) return (Number(m[1])+Number(m[2]))/2; const s=r.match(/(\d+)\s*s/); if(s) return Number(s[1]); return 0; }
function hasEquipment(ctx: Ctx, need: "bar"|"db"|"bench"|"machine"|"kb"|"bands"|"trx"){ if(ctx.equip==="full") return true; switch(need){ case "db": return !!ctx.equipItems.db; case "bar": return !!ctx.equipItems.bar; case "bench": return !!ctx.equipItems.bench; case "machine": return false; case "kb": return !!ctx.equipItems.kb; case "bands": return !!ctx.equipItems.bands; case "trx": return !!ctx.equipItems.trx; default: return false; } }
function matchesPreference(name:string,list?:string[]){ if(!list||list.length===0) return false; const n=name.toLowerCase(); return list.some(k=>n.includes(k.toLowerCase())); }
function estimateTotalMinutes(list:NormalizedExercise[], goal:string){ return list.reduce((sum,e)=>sum+estimateExerciseMinutes(e,goal),0); }

/* ========================= Musculation ========================= */
function buildStrengthFocused(ctx: Ctx, focus: StrengthFocus, goalKey: string, profile?: ProfileInput): NormalizedExercise[] {
  const g=(goalKey||"general").toLowerCase(); const out:NormalizedExercise[]=[];
  ...
  const likes=profile?.likes||[], dislikes=profile?.dislikes||[], pool=(POOLS[focus]||[]).slice();
  const filtered=pool.filter(p=>{ if(p.contra?.(ctx.injuries)) return false; if(p.need && !p.need.every(n=>hasEquipment(ctx,n))){ if(!p.fallback) return false; } if(matchesPreference(p.name,dislikes)) return false; return true; });
  const kindRank:Record<NonNullable<PoolItem["kind"]>,number>={ main:0, assist:1, iso:2, core:3 };
  filtered.sort((a,b)=>{ const likeA=matchesPreference(a.name,likes)?-1:0; const likeB=matchesPreference(b.name,likes)?-1:0; if(likeA!==likeB) return likeA-likeB; return (kindRank[a.kind||"assist"]??9)-(kindRank[b.kind||"assist"]??9); });
  const targetMin=clamp(ctx.minutes,20,90); let usedMin=0;
  for(const p of filtered){
    ...
    const base: NormalizedExercise = {
      name: usableName, sets: defaultSets(ctx.level, isBW), reps: defaultReps(g, isMain),
      rest: g==="strength"?"120–150s":g==="fatloss"?"45–60s":isMain?"75–90s":"60–75s",
      tempo: tempoFor(ctx.goalKey), rir: rirFor(ctx.level), block: "principal",
    };
    const mins=estimateExerciseMinutes(base,g); if(usedMin+mins>targetMin+4) continue;
    out.push(adjustForInjuries(ctx, base)); usedMin+=mins;
    const mains=out.filter(e=>/Squat|Presse|Soulevé|Développé|Row(?!.*serviette)|Tirage|Tractions|Hip Thrust|Bench/i.test(e.name)).length;
    if(usedMin>=targetMin*0.7 && mains>=3){ if(targetMin-usedMin>=5){ out.push(adjustForInjuries(ctx,{ name:"Gainage planche", sets:2, reps:"30–45s", rest:g==="fatloss"?"30–45s":"45–60s", block:"fin" })); } break; }
...
function adjustForInjuries(ctx: Ctx, ex: NormalizedExercise): NormalizedExercise {
  const e:{[k:string]:any}={...ex}; if(e.sets && !e.rir) e.rir=rirFor(ctx.level); if(e.sets && !e.tempo) e.tempo=tempoFor(ctx.goalKey);
  if(ctx.injuries.back){ if(/back squat|soulevé de terre|deadlift|row à la barre/i.test(e.name)) return swap(e, preferBackFriendly(e,ctx));
    if(/superman/i.test(e.name)) e.notes=joinNotes(e.notes,"Si gêne au dos, réduire l’amplitude ou remplacer par Bird-Dog."); }
  if(ctx.injuries.shoulder){ if(/militaire|overhead|développé militaire|élevations latérales lourdes/i.test(e.name))
    ...
    if(/dips/i.test(e.name)) return swap(e,{ name:"Pompes surélevées", sets:e.sets??3, reps:pickBodyweight(ctx.goalKey), rest:"60–75s", block:"principal", equipment:"poids du corps" }); }
  if(ctx.injuries.knee){ if(/sauté|jump|burpee/i.test(e.name))
    ...
    if(/squat(?!.*goblet)|fente/i.test(e.name)) e.notes=joinNotes(e.notes,"Amplitude contrôlée, pas de douleur, option appui/assistance."); }
  if(ctx.injuries.wrist && /pompes|push-up/i.test(e.name)) e.notes=joinNotes(e.notes,"Utiliser poignées de pompe ou poings fermés pour garder le poignet neutre.");
  if(ctx.injuries.hip && /squat|fente/i.test(e.name)) e.notes=joinNotes(e.notes,"Amplitude confortable, focus stabilité hanche.");
  if(ctx.injuries.ankle && /sauté|jump/i.test(e.name)) return swap(e,{ name:"Marche rapide inclinée", sets:e.sets??3, reps:"2–3 min", rest:"60s", block:"principal" });
  if(/tirage élastique|row|tirage/i.test(e.name)){ /* marquage équipement */ }
  if(/kettlebell|kb/i.test(e.name) && !(ctx.equipItems as any).kb) return swap(e,{ ...e, name:e.name.replace(/kettlebell|KB/i,"haltère"), equipment:"haltères" });
  if(/trx|suspension/i.test(e.name) && !(ctx.equipItems as any).trx) return swap(e, bodyOrBand("Tirage élastique / serviette", ctx, { reps: e.reps || pickBodyweight(ctx.goalKey) }));
  return e as NormalizedExercise;
}
...
/* ========================= Exercices helpers ========================= */
function setsFor(level:"debutant"|"intermediaire"|"avance", bw=false){ if(bw) return level==="avance"?4:3; return level==="avance"?4:level==="intermediaire"?3:2; }
function rirFor(level:"debutant"|"intermediaire"|"avance"){ return level==="avance"?1:2; }
function tempoFor(goal:string){ const g=goal.toLowerCase(); if(g==="hypertrophy") return "3011"; if(g==="strength") return "21X1"; return "2011"; }
function repsFor(goal:string){ const g=goal.toLowerCase(); if(g==="strength") return "4–6"; if(g==="hypertrophy") return "8–12"; if(g==="fatloss") return "10–15"; if(g==="endurance") return "12–15"; if(g==="mobility") return "8–10"; return "8–12"; }
function pickBodyweight(goal:string){ const g=goal.toLowerCase(); if(g==="strength") return "6–8"; if(g==="fatloss") return "12–20"; return "10–15"; }
