...
// JS pur (pas de types). Logique : nb de séances = nb de jours annoncés.
// Titres lisibles : “Séance pour Prénom — Lundi”, “— Dimanche”, etc.
// 1 seule séance sans jour explicite → pas de suffixe.

import { getAnswersForEmail as _getAnswersForEmail, buildProfileFromAnswers as _buildProfileFromAnswers } from "../ai";

/* ========================= API principale ========================= */
export function planProgrammeFromProfile(profile = {}, opts = undefined) {
...
export async function planProgrammeFromEmail(email, opts = {}) {
  ...
  let built = {};
  if (last) {
    built = _buildProfileFromAnswers(last) || {};
...
export function planProgrammeFromAnswers(answers, opts = {}) {
  const built = _buildProfileFromAnswers(answers) || {};
...
/** Liste ordonnée des jours trouvés (week-end → samedi+dimanche). */
...
function adjustForInjuries(ctx, ex) {
  ...
  if (ctx.injuries.back) {
    if (/back squat|soulevé de terre|deadlift|row à la barre/i.test(e.name)) return swap(e, preferBackFriendly(e, ctx));
