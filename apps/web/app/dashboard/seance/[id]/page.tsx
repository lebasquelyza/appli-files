...
async function loadData(
  ...
  lang: Lang,
): Promise<{ base: AiSession; exercises: NormalizedExercise[]; focus: Focus; plannedMin: number }> {
  ...
  if (answers) {
    ...
    base =
      (qpTitle &&
        regen.find((s) => stripVariantLetter(s.title) === stripVariantLetter(qpTitle))) ||
      undefined;
  ...
  if (!base) {
    base = {
      id,
      title: tServer(lang, "settings.seancePage.fallbackTitle", "Séance personnalisée"),
      date: "",
      type: "muscu",
      plannedMin: 45,
    } as AiSession;
...
export default async function Page({
  ...
  const lang = getLang();
  const notFoundMsg = tServer(lang, "settings.seancePage.errors.notFound", "Séance introuvable");

  const id = decodeURIComponent(params?.id ?? "");
  if (!id && !(searchParams?.title || searchParams?.type)) {
    redirect(`/dashboard/profile?error=${encodeURIComponent(notFoundMsg)}`);
  }

  const { base, exercises, focus, plannedMin } = await loadData(id, searchParams, lang);
  if (!base) {
    redirect(`/dashboard/profile?error=${encodeURIComponent(notFoundMsg)}`);
  }
  ...
  qs.set("generate", "1");
  if (equip === "none") qs.set("equip", "none");
  ...
  // ✅ Retour: home vs profile
  const from = String(getParam("from") || "").toLowerCase(); // "home" | "profile" | ""
  const backHref = from === "home" ? "/dashboard" : `/dashboard/profile${qs.toString() ? `?${qs.toString()}` : ""}`;

  // ✅ NEW: liens "Séance faite" / "Plus tard" -> profile avec saved/later mis à jour
  const key = id; // on utilise l'id de l'URL comme clé (ex: "s3" ou id réel)

  const addToCsv = (csv: string | undefined, value: string) => {
    const list = (csv ? csv.split(",") : []).map((x) => x.trim()).filter(Boolean);
    if (!list.includes(value)) list.push(value);
    return list.join(",");
  };

  const qsDone = new URLSearchParams(qs.toString());
  qsDone.set("saved", addToCsv(saved, key));
  const saveDoneHref = `/dashboard/profile${qsDone.toString() ? `?${qsDone.toString()}` : ""}`;

  const qsLater = new URLSearchParams(qs.toString());
  qsLater.set("later", addToCsv(later, key));
  const saveLaterHref = `/dashboard/profile${qsLater.toString() ? `?${qsLater.toString()}` : ""}`;
  ...
  return (
    <SeancePageViewClient
      ...
      backHref={backHref}
      saveDoneHref={saveDoneHref}
      saveLaterHref={saveLaterHref}
    />
  );
}
