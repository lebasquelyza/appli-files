...
type Workout = {
  id: string; title: string; type: WorkoutType; status: WorkoutStatus;
  date: string; plannedMin?: number; startedAt?: string; endedAt?: string; note?: string; createdAt: string;
};
type Store = { sessions: Workout[] };

type AiSession = {
  id: string; title: string; type: WorkoutType; date: string;
  plannedMin?: number; note?: string; intensity?: "faible" | "modérée" | "élevée"; recommendedBy?: string;
};
...
async function getSignedInEmail(): Promise<string> {
  ...
  // Cookie (fallback lecture seulement)
...
function fmtDateISO(iso?: string) { if (!iso) return "—"; try { const d=new Date(iso); if(!isNaN(d.getTime())) return d.toLocaleString("fr-FR",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"});} catch{} return iso||"—"; }
function fmtDateYMD(ymd?: string) { if (!ymd) return "—"; try { const [y,m,d]=ymd.split("-").map(Number); const dt=new Date(y,(m||1)-1,d||1); return dt.toLocaleDateString("fr-FR",{year:"numeric",month:"long",day:"numeric"});} catch{} return ymd; }
function minutesBetween(a?: string, b?: string) { if (!a||!b) return; const A=new Date(a).getTime(), B=new Date(b).getTime(); if(!isFinite(A)||!isFinite(B)) return; const mins=Math.round((B-A)/60000); return mins>=0?mins:undefined; }
...
async function fetchValues(sheetId: string, range: string, apiKey?: string) {
  ...
  if (!res.ok) { const txt = await res.text().catch(() => ""); throw new Error(`SHEETS_${res.status}${txt ? ":" + txt.slice(0, 60) : ""}`); }
  return res.json();
}
type Answers = Record<string,string>;
function norm(s:string){return s.trim().toLowerCase().replace(/\s+/g," ").replace(/[éèêë]/g,"e").replace(/[àâä]/g,"a").replace(/[îï]/g,"i").replace(/[ôö]/g,"o").replace(/[ùûü]/g,"u").replace(/[’']/g,"'");}
async function getAnswersForEmail(email:string, sheetId:string, range:string, apiKey?:string): Promise<Answers|null>{
  ...
  for (let i=1;i<values.length;i++){
    ...
    if (cell && cell === email.trim().toLowerCase()){
      const rec: Answers = {}; headers.forEach((h,j) => { rec[h] = (row[j] ?? "").trim(); }); return rec;
    }
  }
  return null;
}
function generateSessionsFromAnswers(ans: Answers): AiSession[] {
  ...
  const age = Number((get("age")||"").replace(",",".")); const poids = Number((get("poids")||"").replace(",",".")); const taille = Number((get("taille")||"").replace(",",".")); 
  const niveau = get("niveau").toLowerCase() || "débutant"; const objectif = get("objectif").toLowerCase();
  ...
  let freq=3; const digits=dispo.match(/\d+/g); if (digits?.length) freq=Math.max(1,Math.min(6,parseInt(digits[0],10)));
  ...
  let pool:WorkoutType[]=["cardio","hiit","mobilité"]; if (muscuPossible) pool=["muscu","cardio","hiit"];
  ...
  const noteParts:string[]=[]; if (prenom) noteParts.push(`Pour ${prenom}`);
  if (isFinite(poids)&&isFinite(taille)&&taille>0){ const imc=Math.round((poids/Math.pow(taille/100,2))*10)/10; if (isFinite(imc)) noteParts.push(`IMC: ${imc}`); }
  if (noEquip) noteParts.push("Sans matériel"); if (atGym) noteParts.push("Salle");
  const today=new Date(); const nb=Math.max(1,Math.min(6,freq)); const sessions:AiSession[]=[];
  for (let i=0;i<nb;i++){ const date=new Date(today); date.setDate(today.getDate()+i*Math.ceil(7/nb));
    const y=date.getFullYear(), m=String(date.getMonth()+1).padStart(2,"0"), d=String(date.getDate()).padStart(2,"0");
    const type=pool[i%pool.length];
    sessions.push({ id:`ai-${y}${m}${d}-${i}`, title:`${objectif.includes("perte")?"Brûle-graisse":objectif.includes("prise")||objectif.includes("force")?"Force/Muscu":objectif.includes("endurance")||objectif.includes("cardio")?"Endurance":"Full body"} — Séance ${i+1}`,
      type, date:`${y}-${m}-${d}`, plannedMin:baseMin, intensity, note: noteParts.join(" · ") || undefined, recommendedBy:"AI" });
  }
  return sessions;
}
...
async function buildProgrammeAction() {
  ...
  // 0) Détecte l'email via NextAuth/cookie (PAS d'écriture de cookie ici, sauf si trouvé)
  ...
  const endpoints = [`${API_BASE}/api/programme/build`, `${API_BASE}/api/program/build`, `${API_BASE}/api/sessions/build`];
  for (const url of endpoints) {
    try {
      const res = await fetch(`${url}?user=${encodeURIComponent(uid)}`, {
        method: "POST",
        headers: { Accept: "application/json", "Content-Type": "application/json", ...(API_KEY ? { Authorization: `Bearer ${API_KEY}` } : {}) },
  ...
  // 2) Fallback Google Sheets — nécessite l'email
  ...
  try {
    ...
    const mapped: Workout[] = aiSessions.map(s => ({
      id: s.id, title: s.title, type: s.type, status: "active", date: s.date,
      plannedMin: s.plannedMin, note: s.note, createdAt: now,
    }));

    const next: Store = { sessions: [...mapped, ...store.sessions].slice(0, 300) };
    jar.set("app_sessions", JSON.stringify(next), { path: "/", sameSite: "lax", maxAge: 60*60*24*365, httpOnly: false });
...
async function addSessionAction(formData: FormData) {
  ...
  const w: Workout = {
    id: uid(), title, type: (["muscu","cardio","hiit","mobilité"].includes(type) ? type : "muscu") as WorkoutType,
    status:"active", date, plannedMin: plannedMinStr ? Number(plannedMinStr) : undefined,
    startedAt: startNow ? new Date().toISOString() : undefined, note: note || undefined, createdAt: new Date().toISOString(),
  };
...
async function completeSessionAction(formData: FormData) {
  ...
  const sessions = store.sessions.map(s => s.id!==id? s : ({ ...s, status:"done" as WorkoutStatus, startedAt: s.startedAt || nowISO, endedAt: nowISO }));
...
export default async function Page({
  ...
  // IMPORTANT: on NE set PAS de cookie ici. On ne fait que LIRE.
  const emailDetected = await getSignedInEmail();
  const emailFromCookie = cookies().get("app_email")?.value || "";
  const emailForLink = emailDetected || emailFromCookie;
  const questionnaireUrl = emailForLink ? `${QUESTIONNAIRE_BASE}?email=${encodeURIComponent(emailForLink)}` : QUESTIONNAIRE_BASE;
          ...
          <form action={addSessionAction} className="grid gap-6 lg:grid-cols-3">
            ...
            <div>
              ...
              <select className="input" name="type" defaultValue="muscu" required>
                <option value="muscu">Muscu</option><option value="cardio">Cardio</option><option value="hiit">HIIT</option><option value="mobilité">Mobilité</option>
              </select>
            </div>
            <div>
