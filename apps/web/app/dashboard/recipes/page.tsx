...
async function generateRecipes({
  ...
  if (typeof kcalTarget === "number" && !isNaN(kcalTarget)) {
    ...
    if (hasMin && hasMax)
      constraints.push(`- Respecter une plage calorique ${kcalMin}-${kcalMax} kcal.`);
  ...
  if (allergens.length)
    constraints.push(`- Exclure strictement ingrédients/allergènes: ${allergens.join(", ")}.`);
...
export default async function Page({
  ...
  const score = (r: Recipe) =>
    r.goals.reduce((n, g) => n + (goalSet.has(String(g).toLowerCase()) ? 1 : 0), 0);
      ...
      <div className="section" style={{ marginTop: 12 }}>
        <div className="section-head" style={{ marginBottom: 8 }}>
          ...
          <div className="text-sm" style={{ color: "#6b7280" }}>
            Ajustez et régénérez les suggestions
          </div>
        </div>
        ...
        <form method="GET" className="grid gap-6 lg:grid-cols-2">
          <div>
            ...
            <input
              ...
              name="kcal"
              placeholder="ex: 600"
              defaultValue={hasKcalTarget ? String(kcal) : ""}
            />
            <div className="text-sm" style={{ color: "#6b7280", marginTop: 6 }}>
              Ou précisez une plage ci-dessous.
            </div>
          </div>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                name="kcalMin"
                placeholder="ex: 450"
                defaultValue={hasKcalMin ? String(kcalMin) : ""}
              />
            </div>
            <div>
              ...
              <input
                ...
                name="kcalMax"
                placeholder="ex: 700"
                defaultValue={hasKcalMax ? String(kcalMax) : ""}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              defaultValue={(searchParams?.allergens ?? "") as string}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              type="text"
              name="diets"
              placeholder="vegan, sans-gluten, halal"
              defaultValue={(searchParams?.diets ?? "") as string}
            />
          </div>
          ...
          <div className="flex items-center justify-between lg:col-span-2">
            <div>
              ...
              {goals.length > 0 && (
                <span className="text-sm" style={{ marginLeft: 10 }}>
                  Objectifs:{" "}
                  {goals.map((g) => (
                    <span key={g} className="badge" style={{ marginRight: 6 }}>
                      {g}
                    </span>
                  ))}
                </span>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <a href="/dashboard/recipes" className="btn btn-outline">
                Réinitialiser
              </a>
  ...
  return (
    <article className="card" style={{ overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
        <a className="btn btn-dash" href={`/dashboard/recipes/${r.id}`}>
          Voir la recette
        </a>
...
function LockedCard({ r, userPlan }: { r: Recipe; userPlan: Plan }) {
  ...
  const needUpgrade =
    targetPlan === "PLUS" && userPlan === "BASIC"
      ? "PLUS"
  ...
  return (
    <article className="card" style={{ position: "relative", overflow: "hidden", opacity: 0.9 }}>
      <div
        style={{ position: "absolute", inset: 0, background: "linear-gradient(transparent, rgba(0,0,0,.04))" }}
      />
      <div className="flex items-center justify-between">
        <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>{r.title}</h3>
        <span className="badge">{r.minPlan}</span>
