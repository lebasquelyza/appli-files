...
/* ============ Validation image (HTTPS ou data URL) ============ */
...
function sanitizeImageInput(raw: string): SanitizeResult {
  ...
  if (/^https:\/\/.+/i.test(s)) return { ok: true, url: s, kind: "https" }; // on force https

  // data URL stricte (png/jpeg/jpg/webp)
...
/* ============ POST — SSE + Chat Completions only ============ */
export async function POST(req: NextRequest) {
  try {
    const ctype = (req.headers.get("content-type") || "").toLowerCase();
    if (!ctype.includes("application/json")) {
      return jsonError(415, "JSON attendu: { frames: base64[]|url[], timestamps?: number[], feeling?: string, fileUrl?: string, economyMode?: boolean, selftest?: boolean }");
    }
    ...
    // --- mode SELFTEST : image 1x1 PNG connue (élimine tout souci d’URL)
    ...
    // --- cache (désactivé en selftest)
    const key = selftest ? "selftest" : hashKey(frames, feeling || "", economyMode);
    ...
    if (fileUrl) userTextParts.push(`URL vidéo: ${fileUrl}`);
    ...
    const ping = () => writer.write(enc.encode(`: heartbeat\n\n`));
    const hb = setInterval(ping, 1000);
    ...
    (async () => {
      try {
            ...
            messages: [
              { role: "system", content: "Réponds STRICTEMENT en JSON." },
              {
                ...
                content: [
                  ...
                  // IMPORTANT: schéma Chat Completions attendu
        ...
        const payload = { code: "openai_error", status, message: String(e?.message || e), details: e?.details };
    ...
    if (Number.isInteger(status)) return jsonError(status as number, msg);
