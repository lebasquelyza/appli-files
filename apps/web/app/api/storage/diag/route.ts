...
export async function GET() {
  try {
    ...
    if (!okUrl) return NextResponse.json({ ok: false, step: "env", url, anon, service }, { status: 500 });

    const admin = getSupabaseAdmin(); // lazy init (throw si env manquant)
    // test rapide : génère une URL d'upload signée factice (pas d'upload réel)
    const { data, error } = await admin.storage.from("videos").createSignedUploadUrl(`diag/${Date.now()}.txt`);
